...
import { getFeaturedCodeSamples, getProjectById } from "@/content";
...
export function CodeSamplesPreview() {
  const samples = getFeaturedCodeSamples()
    ...
    .map((sample) => {
      ...
      return {
        sample,
        relatedProjectTitle: project?.title ?? "Portfolio engineering pattern",
      };
    });
  ...
  return (
    <section aria-labelledby="code-samples-preview-heading" className="border-line border-b">
      <div className="mx-auto w-full max-w-7xl px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            >
              Independently rewritten implementation patterns
            </h2>

            <p className="text-secondary mt-5 max-w-2xl text-base leading-8">
              Focused examples of API structure, authorization, external integrations, and webhook
              processing, with explanations of failure handling, testing, and production trade-offs.
            </p>
            ...
            <p className="text-muted mt-5 inline-flex items-start gap-2 text-sm leading-6">
              ...
              The examples use synthetic entities and do not reproduce proprietary source code,
              credentials, internal identifiers, or company-specific rules.
            </p>
          </div>
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-2">
          {samples.map(({ sample, relatedProjectTitle }) => (
            <CodeSampleCard
              key={sample.id}
              sample={sample}
