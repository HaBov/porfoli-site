...
export function CodeSampleNavigation({ previousSample, nextSample }: CodeSampleNavigationProps) {
    ...
    >
      {previousSample ? (
        ...
        >
          ...
          <span className="text-foreground mt-2 block font-semibold">{previousSample.title}</span>
        </Link>
      ) : (
      ...
      {nextSample ? (
        ...
        >
          ...
          <span className="text-foreground mt-2 block font-semibold">{nextSample.title}</span>
        </Link>
      ) : null}
    </nav>
  );
