...
export function CodeSampleCard({
  ...
  return (
    <article
      className={cn(
        "border-line bg-surface group hover:bg-elevated relative overflow-hidden rounded-2xl border transition-colors",
        variant === "featured" ? "p-7 sm:p-9" : variant === "compact" ? "p-5" : "p-6",
      )}
    >
      ...
      <Heading
        className={cn(
          ...
          variant === "featured" ? "text-3xl sm:text-4xl" : "text-xl sm:text-2xl",
        )}
      >
        <Link
      ...
      <p className="text-secondary mt-4 text-sm leading-7 sm:text-base">{sample.summary}</p>
      ...
      <dl className="border-line mt-6 grid gap-3 border-t pt-5 text-sm sm:grid-cols-2">
        <div>
          ...
          <dd className="text-primary mt-1 font-medium">{relatedProjectTitle}</dd>
        </div>

        <div>
          <dt className="text-muted text-xs">Pattern level</dt>
