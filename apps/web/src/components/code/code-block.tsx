...
export async function CodeBlock({
  ...
  const renderedCode = await codeToHtml(
    normalizedCode,
    {
      lang: language,
      themes: {
  ...
  const highlightedHtml = addHighlightedLines(
    renderedCode,
    highlightLines,
  );
    ...
    <figure className="border-line bg-surface my-6 overflow-hidden rounded-2xl border">
      <div className="border-line bg-elevated flex min-h-12 items-center justify-between gap-4 border-b px-3 sm:px-4">
        <div className="min-w-0">
          <p className="text-foreground truncate font-mono text-xs sm:text-sm">
            {filename}
          </p>
      ...
      <div
        className={cn(
          "code-block-content overflow-x-auto",
