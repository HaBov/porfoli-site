...
export function CodeSamplesExplorer({
  items,
}: CodeSamplesExplorerProps) {
  ...
  const gridItems = featuredItem
    ? visibleItems.filter(
        (item) =>
          item.sample.id !== featuredItem.sample.id,
      )
    : visibleItems;

  return (
    <section
      aria-labelledby="code-library-heading"
    ...
    >
      <div className="mx-auto w-full max-w-7xl px-5 sm:px-8 lg:px-10">
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="text-secondary mt-4 max-w-2xl text-base leading-8">
              Each page isolates one engineering
              concern, presents independently rewritten
              code, and explains validation, failure
              behavior, tests, and production trade-offs.
            </p>
          </div>
        </div>

        <div
          aria-label="Filter code samples"
          ...
          {filters.map((filter) => {
            ...
            return (
              ...
              >
                ...
                <span
                  aria-hidden="true"
                  className="ml-2 font-mono text-xs opacity-75"
                >
                  {counts[filter.id]}
                </span>
              </button>
        ...
        <p
          aria-live="polite"
          className="text-muted mt-7 text-sm"
        >
          Showing {visibleItems.length} of{" "}
          {items.length} code samples.
        </p>
        ...
        {featuredItem ? (
          <div className="mt-8">
            <CodeSampleCard
              ...
              relatedProjectTitle={
                featuredItem.relatedProjectTitle
              }
              variant="featured"
            />
          </div>
        ...
        {gridItems.length > 0 ? (
          ...
          <div className="border-line bg-surface mt-8 rounded-2xl border p-8 text-center">
            ...
            <p className="text-secondary mt-3 text-sm leading-7">
              Clear the selected filter to return to
